...
app.use(async (ctx, next) => {
  if (env === "production") {
    ...
    ctx.response.set(
      ...
      "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.google-analytics.com; font-src 'self' https://fonts.gstatic.com; style-src 'self' https://cdnjs.cloudflare.com https://fonts.googleapis.com; img-src 'self' https://www.google-analytics.com"
    );
...
app.use(async (ctx, next) => {
  ctx.response.set("max-age", cacheTime);
  await next();
});
app.use(
  serve(path.resolve(__dirname, "../www/assets"), { maxage: cacheTime * 1000 })
);
app.use(
